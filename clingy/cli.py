#!/usr/bin/env python3
"""
Main CLI entry point for clingy
...
def main():
    ...
    if not ctx.has_project and not is_init_command:
        log_error("No clingy project found.")
        log_info("Run 'clingy init' to create a new project, or")
        log_info("navigate to a directory with .clingy marker file")
        sys.exit(1)
...
def check_required_dependencies() -> bool:
    ...
    log_info("Run 'clingy requirements' for details")
    return False
...
def cli_mode(ctx):
    ...
    parser = argparse.ArgumentParser(
        description="  Clingy - Context-aware CLI framework with fuzzy search menus",
...
def interactive_mode(ctx):
    ...
    from clingy.core.menu import MenuNode, MenuRenderer

    log_header("CLINGY - INTERACTIVE MODE")
