...
from manager.config import SERVERLESS_STAGE, SERVERLESS_PROFILE, GO_FUNCTIONS, BIN_DIR
...
"""
        ...
        return MenuNode(
            ...
            children=[
                MenuNode(
                    ...
                    emoji="🌐",
                    action=self._deploy_full,
                ),
                MenuNode(
                    label="Deploy Specific Functions",
                    emoji="🎯",
                    action=self._interactive_deploy,
                ),
                MenuNode(
                    label="Full Pipeline (Build → Zip → Deploy)",
                    emoji="⚡",
                    action=self._full_pipeline,
                ),
            ],
        )
    ...
    def _deploy_full(self) -> bool:
        """Deploy full stack"""
        args = Namespace(debug=False, all=False, function=None, function_list=None)
        return self.execute(args)

    def _interactive_deploy(self) -> bool:
        """Selección interactiva de funciones para deploy"""
        functions = fzf_select_functions()
        if not functions:
            return False
        args = Namespace(debug=False, all=False, function=None, function_list=functions)
        return self.execute(args)

    def _full_pipeline(self) -> bool:
        """Pipeline completo: Build → Zip → Deploy"""
        functions = fzf_select_functions()
        if not functions:
            return False
        args = Namespace(debug=False, all=True, function=None, function_list=functions)
        return self.execute(args)
