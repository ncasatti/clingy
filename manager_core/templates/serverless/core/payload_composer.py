    ...
    def load_json_safe(self, path: Path) -> dict:
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                if not isinstance(data, dict):
                    raise PayloadError(
                        f"JSON must be a dictionary, got {type(data).__name__}"
                    )
            ...
            raise PayloadError(
                f"Invalid JSON in {path.name} at line {e.lineno}, "
    ...
    def compose(self, selected_file: Path, stage: str) -> ComposedPayload:
        ...
        if context_path.exists():
            ...
            if context_files:
                ...
                warnings.append(
                    f"Context for stage '{stage}' not found. "
        ...
        if (selected_file.parent != self.payloads_dir and 
            not selected_file.parent.name.startswith("_")):
            ...
            if metadata_path.exists():
                ...
                warnings.append(
                    f"Optional metadata file not found: "
                    f"{function_dir.name}/_metadata.yaml"
                )
    ...
    def validate(self, payload: dict) -> ValidationResult:
        ...
        if "body" in payload:
            ...
            if not isinstance(body, (str, dict, list, type(None))):
                errors.append(
                    f"Body must be string, dict, list, or null. "
